...
/*
Declare a function 'deepObject' which takes in an arbitrarily nested 
object and a number called 'depth' as arguments. Return values of the 
non-empty object at the 'depth' level (if there's more than one valid 
result, return any of them). If nothing found, return "No non-empty 
objects at the level *depth*!".
...
console.log(deepObject(obj, 2)); // { q: 'depth 2 - key q' } OR 
{ e: 'depth 2 - key e', f: 'depth 2 - key f', g: { h: { }, 
i: 'depth 3 - key i', j: { }, k: { l: 'depth 4 - key l' } }, m: 'depth 2 - key m' }
console.log(deepObject(obj, 3)); 
// { h: { }, i: 'depth 3 - key i', j: { }, k: { l: 'depth 4 - key l' } }
*/

// Create helper function to recursively search for non-empty objects
// if the current depth matches the desired depth, return the object
// if the current depth doesn't match yet, continue searching
// if a non-empty object is found, return it
// if no non-empty objects are found, return undefined

function deepObject(obj, depth) {
   
    //******* Helper function *****
    function searchDepth(obj, currentDepth) {
        //if current depth matches desired depth
        if (currentDepth === depth){
            //check if object at that depth is empty by checking length prop
            ...
            // if current object doesn't match yet, continue searching deeper
            for (const key in obj) {
                //check if that key's value is an object itself & if its not empty
                if (typeof obj[key] === 'object' && obj[key] !== null) {
                    //if so pass it to recursive function
                    const result = searchDepth(obj[key], currentDepth + 1);
                    if (result != null) {
                        return result;
    ...
    //Start search from the top-level object
    const result = searchDepth(obj, 1);

    if (result !== null) {
        return result;
    } else {
        return `No non-empty objects at the level ${depth}!`;
    }
}
